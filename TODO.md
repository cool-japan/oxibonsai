...
> 619 source files, ~177,000 lines of Rust code, 4,671 tests passing across workspace — verified 2026-06-08 (0.2.3)
...
| Phase 26: CUDA FP8 E4M3/E5M2 Batch Prefill | ✅ Complete | 8 NVRTC kernels (E4M3/E5M2 × gemm/gemm_residual/fused_gate_up_swiglu/gemv_pf); `try_cuda_prefill_fp8`; `CudaFP8PrefillLayerParams`; `forward_cuda_fp8.rs` with `try_cuda_prefill_with_lm_head_fp8`/`try_cuda_prefill_verify_fp8`; handle namespaces E4M3 26M-31M, E5M2 28M-33M; replaces sequential FP8 CUDA GEMV loop for prefill |
| Phase 25: K-quant CUDA Batch Prefill | ✅ Complete | 18 NVRTC kernels (Q2K/Q3K/Q4K/Q5K/Q6K/Q8K × gemm/gemm_residual/fused_gate_up_swiglu); `try_cuda_prefill_k_quant`; `KQuantFormat` enum; 42 K-quant block accessor methods; replaces sequential CUDA GEMV loop for all K-quant prefill |
| Phase 24   | CUDA Q4_0/Q8_0 Batch Prefill | ✅ Complete | Q4_0/Q8_0 batch GEMM prefill (CPU-only tests pass) |
...
| Phase 20: CUDA Ternary Batch Prefill + FP8 GEMV | ✅ Complete | `try_cuda_prefill_ternary`: TQ2_0_g128 fused batch GEMM kernels (`gemm_tq2_g128_v7`, `gemm_tq2_g128_v7_residual`, `fused_gate_up_swiglu_gemm_tq2`) with cap-of-8 outer loop, SoA weight layout, per-token `encode_attn_phase_tq2`; replaces sequential CUDA prefill fallback for Ternary models. `cuda_gemv_fp8_e4m3`/`cuda_gemv_fp8_e5m2`: NVRTC FP8 GEMV kernels with AoS block decode, warp-per-row reduction; `KernelDispatcher::Gpu` tier dispatches to GPU path for both E4M3 and E5M2; host-only + CI-GPU-gated parity tests. |
