...
fn oxi_tokenizer_with_unigram_encodes() {
    ...
    let tok = make_tokenizer(&[("<unk>", 0.0), ("hello", -1.0), (" ", -0.5), ("world", -1.0)], 0);
...
fn oxi_tokenizer_is_unigram_flag() {
    ...
    assert!(!bpe_tok.is_unigram(), "BPE stub should report is_unigram == false");
}
...
fn unigram_encode_unknown_chars() {
    ...
    let ids = tok.encode("z").expect("encode should succeed with UNK fallback");
    // UNK token is id=0.
...
fn unigram_bos_eos_injection() {
    ...
    assert_eq!(*ids.last().expect("must have last"), 11, "last token should be EOS");
}
...
fn unigram_max_length_truncates() {
    ...
    assert!(ids.len() <= 2, "expected truncation to 2 tokens, got {ids:?}");
}
